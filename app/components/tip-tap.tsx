...
const MenuBar = ({ editor }: { editor: Editor }) => {
  ...
  return (
    <div className='flex w-full flex-grow  flex-row items-center justify-between rounded-t-md border-b-2 border-gray-200 bg-gray-100 p-2 text-white dark:border-gray-700 dark:bg-gray-800'>
      <button
        type='button'
...
const TipTap = ({ content }: { content?: string }) => {
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class:
          'flex-1 p-4 mx-auto border border-t-0 text-black dark:text-slate-50 w-full h-auto text-sm m-5 focus:outline-non rounded-b-md mt-0 '
      }
    }
  })
